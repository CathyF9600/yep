...
static void _exit_cleanup(void) {
    ...
    if (TAILQ_EMPTY(&list_head)) {
        ...
        //     each->queue_entry = NULL;
        //     free(each);
        // }
        each = TAILQ_FIRST(&tcb_queue_head);
        while (each) {
        TAILQ_REMOVE(&tcb_queue_head, each, entries);
        if(each->init == 0){
            delete_stack(each->context->uc_stack.ss_sp);
        }
        free(each->context); //free context memory
        each->context = NULL; //set context pointer to NULL to flag this memory as having been freed. Do this whenever you free a ucontext pointer
        free(each->queue_entry); //free the memory associated with the queue entry (the thread is gone for good)
        each->queue_entry = NULL; //set the queue struct pointer to NULL to flag that it's been freed
        free(each); //free the tcb struct
        each = TAILQ_FIRST(&tcb_queue_head); //get next item on TAILQ of TCBs
        }
        exit(0);
    }
...
int wut_join(int id) {
    ...
    struct tcb *cur;
    TAILQ_FOREACH(cur, &tcb_queue_head, entries) {
        if (cur->tid == id) {
            break;
        }
    }
    /* Check if the current thread is waiting on another thread or waiting by anotehr thread */
    if (cur->waiter_id != -1 || cur->waiting_id != -1) {
        return -1;
    }
    ...
    if (waited->has_exited == 0) {
        ...
        TAILQ_REMOVE(&list_head, cur->queue_entry, entries);
        ...
        int er = swapcontext(waited->context, cur->context);
        if (er == -1) {die("swapcontext failed.");}
    }
    else { //waited thread has exited explicitly
        /* Update status */
        cur->waiting_id = -1;
        waited->waiter_id = -1;
        /* Remove the exited thread from tcb queue */
        TAILQ_REMOVE(&list_head, entry, entries);
        if (waited->init == 0) {
...
void wut_exit(int status) {
    ...
    if (current_thread->waiter_id != -1){
        ...
        TAILQ_FOREACH(waiter_thread, &tcb_queue_head, entries) {
            if(waiter_thread->tid == current_thread->waiter_id){
                ...
                TAILQ_INSERT_TAIL(&list_head, waiter_thread->queue_entry, entries);
                break;
                // update of waiting relationship will be done in join
            }
        }
    }
    ...
    int err = setcontext(nxt_thread->context);
    if (err == -1) {
        die("fail to cleanup after the last thread.");
    }
